...
contract Auction is IAuction, Ownable, ReentrancyGuard {
    ...
    function createAuction(
        ...
        require(
            NftContract.balanceOf(msg.sender, id) > 0,
            "does not own NFT"
        );
    ...
    function resolveAuction(uint256 auctionId) external onlyOwner {
        require(
            keccak256(bytes(getAuctionStatus(auctionId))) == keccak256(bytes("ENDED")),
    ...
    function _nftPayment(uint256 auctionId, uint256 fundsToPay, INFT Nft) internal {
